...
The answer, as we show, is that it costs nothing. Local policies with no representation of the global optimum, no communication channel, and no memory reliably converge to $O_{\min} = 3$ in six of eight experimental conditions. No agent computes the global optimum; the pattern provides it for free. This is a free lunch in Levin's sense: the system receives optimal global coordination without paying the computational cost of global optimization.
...
- We provide the first test of Levin's Platonic Space framework using a system where the pattern is a known mathematical theorem ($O_{\min} = m - n$), enabling free-lunch quantification.
...
- We identify the absence of stress inoculation under faithful perception as evidence for pattern-channeling in the clean structural experiments, and we show that delayed gratification, absent under faithful perception, is recruited precisely when the interface corrupts perception (noisy and misleading conditions), marking the boundary between pure pattern-channeling and experience-dependent recovery.
...
The persistence metrics clarify that the pattern, rather than learning, drives this recovery. Same-target retry remains non-zero under damage (0.257) and after healing (0.226), so agents do not form durable avoidance memories. Instead, recovery is a collective state-reconfiguration driven by the pattern: the distribution of pigeons adapts because the pattern $O_{\min}$ shifts with the substrate, and the local policies channel the new pattern.
...
**Key finding.** The absence of stress inoculation is the most theoretically significant result of this experiment. There is no inoculation effect on outcome: both conditions reach identical overload and convergence speed. This contrasts with biological morphogenetic systems, where gradual exposure to stressors can build tolerance (Levin, 2022).

Under the Platonic Space framework, this absence is informative. Stress inoculation requires memory: prior exposure to a stressor triggers adaptive responses that improve tolerance to future exposure. Our pigeons have no memory. Each activation is stateless: the pigeon inspects current loads, applies its policy, and acts. Without memory, there is no substrate for inoculation. Under faithful perception (which holds throughout this experiment), this confirms that the convergence is pattern-channeling: the system does not benefit from experience because it does not accumulate experience, and the free lunch comes from the pattern $O_{\min}$ with no history-dependent adaptation to supply it. We stress that this conclusion is specific to faithful perception. As Experiments 3 and 8 show, once the interface corrupts perception the convergence is no longer monotone and delayed gratification appears; the absence of stress inoculation here therefore demonstrates pattern-channeling in the clean regime, not that experience is irrelevant in every regime.
...
**Classification.** The outcome equivalence is a *pattern invariant*: the final state depends only on the total damage and is indifferent to its temporal profile, because the pattern $O_{\min}$ is fixed by the number of usable holes alone. The absence of stress inoculation confirms *pattern-channeling under faithful perception*: in this clean regime the free lunch comes from the pattern, with accumulated experience contributing nothing. Whether experience contributes once perception is corrupted is answered separately by Experiments 3 and 8.
...
**Free lunch.** In the clean structural experiments, the system receives more than it pays for. The payment is local greedy computation with no global information. The receipt is global optimality, damage compensation, and recovery. The gap between payment and receipt is the free lunch. We note one free lunch we initially expected but did not find: policy-type self-organization in chimeric populations (Experiment 5) does not exceed the correct 0.75 chance baseline, so mixed-policy clustering is not an additional free lunch.
...
**"Patterns are causal."** The mathematical truth $O_{\min} = m - n$ determines system behavior. No agent computes it, yet the system converges to it in 6/8 experiments. The pattern is causal in Pearl's counterfactual sense: if $O_{\min}$ were different (different $m$, different $n$), the system's final state would be different. The pattern determines the outcome; the policy merely accesses it.
...
The most striking finding is the asymmetry between pattern unavailability and pattern corruption. Frozen holes (crash faults) reduce the interface's capacity, but the pattern adapts: $O_{\min}$ increases, and the system channels the new pattern. Misleading holes (Byzantine faults) corrupt the interface, and the same pattern-seeking mechanism that produces the free lunch now produces a free disaster, efficient convergence toward false attractors. The system's efficiency at channeling patterns is its vulnerability under corruption.

The absence of stress inoculation (Experiment 7) and the absence of delayed gratification in the six clean structural experiments (DG Index $= 0.0$ under faithful perception) confirm that the convergence is pattern-channeling when the interface transmits the signal honestly. Under those conditions the free lunch comes from the pattern, the mathematical truth $O_{\min} = m - n$, with accumulated experience contributing nothing. The picture is sharper than uniform pattern-channeling, however: delayed gratification is not absent everywhere. It emerges precisely when the interface corrupts perception: the mean DG Index rises from 0 to between 0.37 and 0.47 under noisy perception (Experiment 3, non-zero in 11 to 16 of 30 runs) and to as high as 0.56 under misleading holes (Experiment 8, non-zero in 22 of 30 runs), where reaching $O_{\min}$ requires climbing out of transiently worse states. The pigeonhole system is thus a clean model in which the pattern contribution can be isolated under faithful perception and the onset of experience-dependent behavior can be located exactly at the point where the interface stops transmitting the signal faithfully.
